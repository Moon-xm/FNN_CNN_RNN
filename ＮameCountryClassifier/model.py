...
class GRUModel(nn.Module):
	def __init__(self, input_size, hidden_size, output_size, num_layers=1, bidirectional=True):
		...
		self.gru = nn.GRU(self.embedding_dim, hidden_size, num_layers,
						  bidirectional=self.bidirectional)  # 双向
		self.fc = nn.Linear(hidden_size * self.bidirectional, output_size)

	def forward(self, x, seq_lenths):
		# x shape: BxS -> SxB  S为seq_len B为batch_size
		x = x.t()
		batch_size = x.size(1)  # B

		hidden = self._init_hidden(batch_size)  # hidden shape see _init_hidden
		embedding = self.embedding(x)  # embedding shape: seq_len x batch_size x embedding_dim

		# pick them up
		gru_input = pack_padded_sequence(embedding, seq_lenths)  # 将序列单元按长度降序排序
